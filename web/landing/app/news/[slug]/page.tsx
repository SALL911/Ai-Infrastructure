...
import { getSeedItem, type SeedNewsItem } from "@/lib/news/seed";
...
function seedToDetail(s: SeedNewsItem): NewsDetail {
  return {
    id: s.id,
    slug: s.slug,
    title_zh: s.title_zh,
    summary_zh: s.summary_zh,
    bci_perspective: s.bci_perspective,
    category: s.category,
...
async function loadItem(slug: string): Promise<NewsDetail | null> {
  ...
  try {
    ...
    const { data } = await sb
      ...
      .eq("status", "published")
      .maybeSingle();
    if (data) return data as NewsDetail;
  } catch {
    // Fall through to seed fallback
  }

  // Fallback to editor-curated seed (Issue #1 content)
  const seed = getSeedItem(slug);
    ...
    <main className="min-h-screen bg-bg text-ink">
      ...
      <article className="mx-auto max-w-3xl px-6 py-12 md:py-16">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2">
          <span className="rounded-full border border-accent/30 bg-accent-soft px-2.5 py-0.5 font-mono text-[10px] font-bold uppercase tracking-[1px] text-accent">
        ...
        <section className="mt-10">
          ...
          <p className="text-base leading-[1.85] text-ink md:text-lg">
            {item.summary_zh}
          </p>
        </section>

        {item.bci_perspective && (
          <section className="mt-10 rounded-card border-l-4 border-accent bg-surface p-6">
            <h2 className="mb-3 font-mono text-xs font-bold uppercase tracking-[2px] text-accent">
              BCI 視角 · Symcio
            </h2>
            ...
            <p className="mt-4 font-mono text-[11px] text-muted-dim">
              依循 Symcio Brand Capital Index 方法論(F · V · E 三軸) ·{" "}
              <Link
                href="/about"
        ...
        <section className="mt-12 rounded-card border border-line bg-surface p-6">
          ...
          <p className="text-sm font-semibold text-ink">{item.source_title}</p>
          {item.source_author && (
        ...
        <section className="mt-12 grid gap-3 md:grid-cols-3">
          ...
          >
            ...
            <div className="mt-2 font-bold text-ink">免費 BCI 診斷</div>
          ...
          >
            ...
            <div className="mt-2 font-bold text-ink">ESG FAQ</div>
            <div className="mt-2 font-mono text-xs text-accent">
              TNFD / IFRS →
            </div>
          </Link>
          ...
          >
            ...
            <div className="mt-2 font-bold text-ink">Discord</div>
            <div className="mt-2 font-mono text-xs text-accent">
              #news 頻道 →
            </div>
          </a>

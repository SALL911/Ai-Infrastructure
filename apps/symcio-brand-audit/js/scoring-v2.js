...
(function (root) {
  ...
  const INDUSTRY_BRAND_ROLE = {
    '食品飲料': 0.75, '科技軟體': 0.60, '金融服務': 0.55,
    '零售電商': 0.80, '製造業': 0.40, '醫療健康': 0.50,
    '教育培訓': 0.65, '餐飲服務': 0.85, '媒體娛樂': 0.90,
    '能源環保': 0.35, '其他': 0.50
  };
  ...
  const INDUSTRY_LEAP = {
    ...
    '教育培訓': 10, '餐飲服務': 68, '媒體娛樂': 8,
    '能源環保': 85, '其他': 30
  };

  const COMPETITORS = {
    '食品飲料': [{ name: '統一', score: 82 }, { name: '鮮乳坊', score: 45 }, { name: '義美', score: 71 }],
    ...
    '媒體娛樂': [{ name: 'KKBOX', score: 60 }, { name: '愛奇藝台灣', score: 45 }, { name: 'friDay', score: 50 }],
    '能源環保': [{ name: '台電', score: 78 }, { name: '中鋼', score: 65 }, { name: '台達電', score: 85 }],
    '其他':     [{ name: '全聯', score: 70 }, { name: 'IKEA台灣', score: 75 }, { name: 'Costco台灣', score: 80 }]
  };

  const REVENUE_MULT = {
    'NTD 500萬以下': 0.3, '500萬-3000萬': 0.5, '3000萬-1億': 0.7,
    '1億-5億': 0.85, '5億以上': 1.0, '不方便透露': 0.5
  };

  const SIZE_MULT = {
    '1-10人': 0.4, '11-50人': 0.6, '51-200人': 0.8, '200人以上': 1.0
  };

  // ---- Main entry ----

  function calculateBCI(data) {
    const {
      brandNameZh = '', brandNameEn = '', website = '',
      industry = '其他', description = '',
      companySize = '1-10人', revenue = '不方便透露'
    } = data;
    ...
    const chatgptScore = Math.min(100, baseAI + hasWebsite + seededRandom(seed + 10, -5, 15));
    const perplexityScore = Math.min(100, baseAI + hasWebsite + seededRandom(seed + 11, -8, 12));
    const googleAIScore = Math.min(100, baseAI + hasWebsite + seededRandom(seed + 12, -3, 18));
    const claudeScore = Math.min(100, baseAI + hasWebsite + seededRandom(seed + 13, -10, 10));
    ...
    if (!geoChecks.wikidata) recommendations.push({
      priority: '高', title: '建立 Wikidata 品牌實體',
      desc: '在 Wikidata 建立品牌結構化實體，讓 AI 引擎的知識圖譜「認識」您的品牌。預估可提升 AI 可見度 15-25%。',
      action: '使用 Symcio Entity Builder 免費工具，5 分鐘完成'
    });
    if (!geoChecks.schemaOrg) recommendations.push({
      priority: '高', title: '嵌入 Schema.org 結構化資料',
  ...
  function bciTier(score) {
    if (score >= 80) return { key: 'excellent', label: '優秀', color: '#2dd4a0' };
    if (score >= 60) return { key: 'good',      label: '良好', color: '#378ADD' };
    if (score >= 40) return { key: 'warning',   label: '需改善', color: '#fbbf24' };
